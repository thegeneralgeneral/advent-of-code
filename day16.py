...
class Aunt(object):
    def __init__(self, aid, children=None, cats=None, samoyeds=None, pomeranians=None, akitas=None,
                 vizslas=None, goldfish=None, trees=None, cars=None, perfumes=None):
        self.aid = aid
    ...
    def __str__(self):
        ...
        result = 'Sue %s (%s)' % (self.aid, data)
        return result

def aunts_match(target, potential_match):
    for field_name in fields:
        f_a = getattr(target, field_name)
        f_b = getattr(potential_match, field_name)
        if field_name in ['cats', 'trees']:
            # "target" reading is less than match
            if f_b is not None and f_a >= f_b:
                return False
        elif field_name in ['pomeranians', 'goldfish']:
            # "target" reading is greater than match
            if f_b is not None and f_b >= f_a:
                return False
        else:
           if f_b is not None and f_b != f_a:
                return False
...
def get_aunt_from_line(line):
    match = re.match(r'Sue (\d+): (.*)', line)
    n, remaining_line = int(match.group(1)), match.group(2)
    fields = {}
    for chunk in remaining_line.split(','):
        match = re.search(r'([a-z]+): (\d+)', chunk)
        name, val = match.group(1), int(match.group(2))
        fields[name] = val
    return Aunt(n, **fields)

TARGET_STRING = """children: 3
cats: 7
samoyeds: 2
pomeranians: 3
akitas: 0
...
INPUT_STRING = """Sue 1: children: 1, cars: 8, vizslas: 7
