def is_nice(input_string):
    vowels = [c for c in input_string if c in ['a', 'e', 'i', 'o', 'u']]
    contains_three_vowels = len(vowels) >= 3
    if not contains_three_vowels:
        return False
    
    contains_doubled_letter = False
    for i in range(len(input_string)-1):
        if input_string[i] == input_string[i+1]:
            contains_doubled_letter = True
            break;
    if not contains_doubled_letter:
        return False
    
    for stop_phrase in ['ab', 'cd', 'pq', 'xy']:
        if stop_phrase in input_string:
            return False
