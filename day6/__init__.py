...
def get_1000_by_1000_grid():
    return [[0 for _ in range(1000)] for _ in range(1000)]

def apply_instruction_string_to_grid(instruction_string, grid):
    result = apply_instruction_to_grid(grid, convert_instruction_string(instruction_string))
    return result

def convert_instruction_string(instruction_string):
    match = re.match(r'(toggle|turn off|turn on) (\d+),(\d+) through (\d+),(\d+)', instruction_string)
    instruction = match.group(1)
    a = (int(match.group(2)), int(match.group(3)))
    b = (int(match.group(4)), int(match.group(5)))
    return instruction, a, b

def apply_instruction_to_grid(input_grid, instruction):
    # Instruction is a tuple of (action, a, b)
    # Returns resulting grid
    result_grid = input_grid
    action, a, b = instruction
    for x in range(a[0], b[0]+1):
        for y in range(a[1], b[1]+1):
            result = apply_action_to_value(action, input_grid[y][x])
            result_grid[y][x] = result
    return result_grid
...
INPUT_STRING = """toggle 461,550 through 564,900
