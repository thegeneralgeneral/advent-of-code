...
def _unquote_string(input_str):
    return input_str[1:-1]

def _convert_hex_chars(input_str):
    result = re.sub(r'\\x([a-f0-9]{2})', r'', input_str)
    return input_str


def get_num_code_chars(input_str):
    return len(input_str)

def get_num_str_chars(input_str):
    print 'Converting %r to string:' % input_str
    result = _unquote_string(input_str)
    print result
    result = result.decode('string-escape')
    print result + '\n'
    return len(result)

def encode_chars(input_code_str):
    print 'Encoding %s' % input_code_str
    result = input_code_str.replace('\\', '\\\\')
    print ' ' + result 
...
def get_num_encoded_chars(input_code_str):
    result = len(encode_chars(input_code_str))
    print 'Encoded length: %s' % result
    return result

INPUT_STRING = r'''"\xa8br\x8bjr\""
"nq"
